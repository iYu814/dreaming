...
if (_cancelEnvEl) _cancelEnvEl.addEventListener('click', () => {
    ...
    if (annToggleBtn && annFormWrapper) {
        ...
        function initDataManagementListeners() {
            ...
            const _cleanOldMsgsEl = document.getElementById('dm-clean-old-msgs');
            if (_cleanOldMsgsEl) {
                _cleanOldMsgsEl.addEventListener('click', () => {
                    if (!confirm(`确定要清理旧消息吗？\n\n将保留最近100条消息，其余消息将被永久删除。\n\n当前共有 ${messages.length} 条消息。`)) return;
                    const removeCount = messages.length - 100;
                    if (removeCount > 0) {
                        messages = messages.slice(-100);
                        showNotification(`已清理 ${removeCount} 条旧消息`, 'success');
                        saveData();
                        renderMessages();
                        const barEl = document.getElementById('dm-storage-bar');
                        if (barEl) barEl.style.width = '0%';
                        const totalEl = document.getElementById('dm-storage-total');
                        if (totalEl) totalEl.textContent = '重新计算…';
                    } else {
                        showNotification('消息数量少于100条，无需清理', 'info');
                    }
                });
            }
...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    if (settings.musicPlayerEnabled) {
        ...
        function initCoreListeners() {
            ...
            const partnerInitBtn = document.getElementById('partner-init-btn');
            if (partnerInitBtn) {
                partnerInitBtn.addEventListener('click', () => {
                    simulateReply();
                    showNotification((settings.partnerName || '对方') + ' 有话想对你说 ~', 'info', 2500);
                });
            }
        }
