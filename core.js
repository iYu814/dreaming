...
function _backupCriticalData() {
    ...
    try {
        ...
        if (msgSizeEstimate > 8 * 1024 * 1024) {
            payloadToStore = {
                ...backupPayload,
                messages: messages.slice(-500),
                _truncated: true
            };
        } else if (msgSizeEstimate > 3 * 1024 * 1024) {
            payloadToStore = {
                ...backupPayload,
        ...
        if (json.length > 4.5 * 1024 * 1024) {
            ...
            try {
                ...
                console.warn('[backup] localStorage 写入失败，尝试精简备份');
                const minimalPayload = {
                    messages: messages.slice(-20),
                    _truncated: true
                };
                try {
                    localStorage.setItem(_BACKUP_PREFIX + 'critical', JSON.stringify(minimalPayload));
                } catch (e2) {
                    console.error('[backup] 备份写入失败:', e2);
                }
            }
        } else {
            try {
...
const addMessage = (message) => {
    ...
    if (messages.length >= MAX_MESSAGES) {
        const removeCount = Math.floor(messages.length * 0.2);
        const removed = messages.splice(0, removeCount);
        if (removed.length > 0 && typeof showNotification === 'function') {
            showNotification(`自动清理了 ${removed.length} 条旧消息`, 'info', 3500);
        }
    }
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            (window.customReplyGroups || []).forEach(g => {
                if (g.disabled && Array.isArray(g.items)) g.items.forEach(item => {
                    disabledGroupItemsOnce.add(typeof item === 'object' ? JSON.stringify(item) : item);
                });
            });
            const replyPoolOnce = customReplies
                .filter(r => {
                    const key = typeof r === 'object' ? JSON.stringify(r) : r;
                    return !disabledItemsOnce.has(key) && !disabledGroupItemsOnce.has(key);
                })
                .map(r => {
                    if (typeof r === 'object') {
                        return String(r.text || r.title || '').trim();
                    }
